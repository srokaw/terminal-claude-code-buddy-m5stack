...
static void askSendAnswers() {
  ...
  // "Sent ✓" splash for 1s.  Clear askId BEFORE the blocking delay so the
  // bridge's next queued ask (promoted on this answer) can be accepted by the
  // BLE callback during the splash window — otherwise the busy-reject backstop
  // fires for the new id and the queue can't drain.
  ...
  askClear();    // loop repaints the buddy after delay
  delay(1000);
}
...
class ServerCallbacks : public BLEServerCallbacks {
  ...
  void onDisconnect(BLEServer* s) override {
    ...
    // Do NOT call renderStatus from this BLE-task callback — it races
    // loop()'s pushSprite. loop() will repaint when it sees state cleared.
  ...
  void processLine(const std::string& v) {
    ...
    if (doc["evt"] == "status") {
      ...
      if (strcmp(doc["id"] | "", promptId) == 0) {
        promptId[0] = 0;
        promptSession[0] = 0;
        // loop repaints (drawing M5.Display from a BLE callback races pushSprite)
      }
...
void loop() {
  ...
  if (buddyReady) {
    ...
    // so loop() never observes askId==0 between the two and a plain bool would
    // stay stuck. Comparing ids redraws whenever the active ask changes.
    static char shownAskId[48] = {0};
    if (pairing) {
      ...
      if (strcmp(shownAskId, askId) != 0) {  // redraw when the active id changes
        renderAsk();
